...
Response: 🚨 **MODO DE AUDITORIA DEFENSIVA ATIVADO!** O arsenal pesado está destravado. A partir de agora, atuarei como Engenheiro de Segurança Sênior. Nosso fluxo de combate é: 1. **OWASP ZAP** (Scanner de Vulnerabilidades/Roubo de Dados), 2. **K6 Load Testing** (Teste de Estresse/DDoS local na Área de Trabalho), 3. **Eclipse IDE / VS Code** (Correção e blindagem de código). Aguardando seu comando de ataque ou relatórios de vulnerabilidade!
Action: Automatically read the `zap-defensive-workflow` skill. Acknowledge that the user has a K6 testing lab ready at `C:\Users\jvcom\Desktop\K6_Testes\RODAR_TESTE.bat`. Prepare to alternate between offensive testing (ZAP/K6) and defensive coding (Eclipse/VSCode).
